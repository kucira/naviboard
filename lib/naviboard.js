...
naviboard.prototype.makeNavigationRulesForComponent = function(obj, row, column, status, activeElement) {
    var self = this;
    ...
    var _getMatrixForNavigation = new Promise((resolve, reject) => {
        ...
        const navRealArray = [];
        sortByXYcoordinate(obj).forEach(item => {
            navRealArray.push([item[0].elementToFocus]);
        })
        resolve(fillNavigationArray(sortByXYcoordinate(obj), navRealArray, threshold_width, threshold_height));
        reject(err);
    });
    ...
    _getMatrixForNavigation.then((matrix) => {
        let indexActiveElement = 0; 
        console.log(activeElement, matrix);
        if(activeElement) { //if update, get the current active element
            const isInputText = activeElement instanceof HTMLInputElement && activeElement.type == 'text';
            console.log(isInputText);
            indexActiveElement = matrix.findIndex(elem => { // find index current active element
                let element = elem[0];
                
                if(isInputText) {
                    if(elem[0].childNodes.length === 2)
                        element = elem[0].childNodes[1]
                    else
                        element = elem[0].childNodes[0]
                }
        ...
        if (self.matrixForNavigation.length != 0 && self.matrixForNavigation != undefined) {
            ...
            if (self.prevComponentRendered.length >= 2 && self.prevActiveElement.length >= 2) {
                if (self.prevComponentRendered[self.prevComponentRendered.length - 2] == self.nextComponentRendered && self.prevComponentRendered[self.prevComponentRendered.length - 2] != null && self.nextComponentRendered != null) {
                    ...
                    if(status === 'update') { // check update navigation
                        if(indexActiveElement !== -1) { // find current index focus
                            //set current index focus
                            // so focus not going back to first element
                            self.currentX = indexActiveElement; 
...
naviboard.prototype.handleView = function(elementIdOfComponentDOM, status) {
    var self = this;
    ...
    _getAllChildNodes.then((children) => {
        ...
        self.makeNavigationRulesForComponent(self.arrayOfCoordinates, row, column, status, activeElement);
    }, (err) => {
        console.log(err);
    });
}
...
naviBoard.setActiveElement = function(elementIdOfComponentDOM, element) {
    var childNodes = [];
    var _getAllChildNodes = new Promise((resolve, reject) => {
        masterElement = document.getElementById(elementIdOfComponentDOM);
        childNodes = masterElement.getElementsByClassName("navigable");
    ...
    _getAllChildNodes.then((children) => {
        ...
        for (var i = 0; i < children.length; i++) {
            children[i].tabIndex = children[i].tabIndex === -1 ? 0 : children[i].tabIndex;
            navRealArray.push(children[i]);
        }

        console.log('-- nav real ---', navRealArray);

        let indexActiveElement = 0; 
        const isInputText = element instanceof HTMLInputElement && element.type == 'text';
        console.log(isInputText);
        indexActiveElement = navRealArray.findIndex(elem => { // find index current active element
            let elementItem = elem;
            
            if(isInputText) {
                if(elem.childNodes.length === 2)
                    elementItem = elem.childNodes[1]
                else
                    elementItem = elem.childNodes[0]
            }
            console.log(elementItem, element);
            console.log(elementItem === element);
